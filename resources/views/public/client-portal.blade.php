        ...
        <div class="pub-auth-card text-center">
            ...
            <form method="POST" action="{{ route('public.client-portal.store') }}" class="pub-contact-form text-start">
                ...
                <div class="form-check mb-3">
                    ...
                    <label for="remember"
                        class="form-check-label small">{{ __('app.public.client_portal.remember_me') }}</label>
                </div>
                <button type="submit"
                    class="pub-btn-gold w-100 justify-content-center">{{ __('app.public.client_portal.login') }}</button>
            </form>

            <p class="small text-muted mt-4 mb-0">
                {{ __('app.public.client_portal.no_account') }}
                <a href="{{ route('public.contact') }}"
                    style="color:var(--pub-gold);font-weight:700;">{{ __('app.public.client_portal.contact_us') }}</a>
            </p>
        </div>
    </div>
</x-public-layout>
